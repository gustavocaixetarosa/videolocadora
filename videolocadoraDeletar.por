...
{	
	...
	funcao deletarFilme(cadeia nomes [], inteiro quantoAlugado [], inteiro anos [], logico disponivel []){
		inteiro indice = 0
		escreva("\n\n### Filmes Cadastrados ###\n")
		para(inteiro i = 0; i < filmesCadastrados; i++){
		...
		escreva("Qual o filme que deseja excluir?\n")
		leia(indice)

		para(inteiro i = indice - 1; i < filmesCadastrados; i++){
			nomes[i] = nomes[i+1]
			quantoAlugado[i] = quantoAlugado[i + 1]
			anos[i] = anos[i + 1]
			disponivel[i] = disponivel[i+1]
		}

		escreva("Filme excluido com sucesso!\n\n")
		filmesCadastrados--
		exibirMenuEDefineOpcao(nomes, quantoAlugado, anos, disponivel)
	}
	...
	funcao exibirMenuEDefineOpcao(cadeia nomes [], inteiro quantoAlugado [], inteiro anos [], logico disponivel []){
		...
		escolha(opcao){
			...
			caso 7:
				deletarFilme(nomes, quantoAlugado, anos, disponivel)
			caso 8:
				sairDoPrograma()
		}
	}
 ...
 * 
 * @POSICAO-CURSOR = 1896; 
 * @DOBRAMENTO-CODIGO = [14, 36, 92, 149, 154, 166];
 * @PONTOS-DE-PARADA = ;
